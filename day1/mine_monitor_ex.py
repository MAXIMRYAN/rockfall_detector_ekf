...
while True:
    ...
    if boxes is not None:
        for box in boxes:
            ...
            # Predict future path
            if len(positions) > 2:
                recent_points = positions[-3:] if len(positions) >= 3 else positions
                dx = recent_points[-1][0] - recent_points[0][0]
                dy = recent_points[-1][1] - recent_points[0][1]
                speed = np.sqrt(dx**2 + dy**2)
                prediction_factor = min(3.0, 1.0 + speed / 30)
                future_x = recent_points[-1][0] + dx * prediction_factor
                future_y = recent_points[-1][1] + dy * prediction_factor

                cv2.circle(annotated_frame, (int(future_x), int(future_y)), 8, (0, 255, 255), -1)
                cv2.line(annotated_frame, positions[-1], (int(future_x), int(future_y)), (0, 255, 255), 2)
