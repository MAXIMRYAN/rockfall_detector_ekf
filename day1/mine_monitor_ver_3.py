# main.py - Simplified without complex ngrok config
import cv2
import numpy as np
...
if __name__ == "__main__":
    ...
    print(f"\n🚀 Starting Mine Rockfall Monitor Server on port {PORT}")
    print(f"📍 Local URL: http://localhost:{PORT}")
    print("👉 Endpoints:")
    print(f"   - http://localhost:{PORT}/video (Live video feed)")
    print(f"   - http://localhost:{PORT}/status (System status)")
    print(f"   - http://localhost:{PORT}/config (Configuration)")
    print(f"\n🌐 To create public URL, run in separate terminal:")
